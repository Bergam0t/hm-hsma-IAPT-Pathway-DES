...
class Model:
    ...
    def __init__(self, run_number):
        ...
        # denotes at risk patient, 1 = Yes, 0 = No
        self.asst_results_df['At Risk'] = [0]
        self.asst_results_df['Referral Time Screen'] = [
            0.0]  # time in mins taken to screen referral
        ...
        # time between screening and getting review
        self.asst_results_df['Review Wait'] = [0]
        self.asst_results_df['Review Rejected'] = [0]  # 1 = Yes, 0 = No
        self.asst_results_df['Opted In'] = [0]  # 1 = Yes, 0 = No
        # time between opt-in notification and patient opting in
        self.asst_results_df['Opt-in Wait'] = [0.0]
        # time between opting in and actual TA, 4 week window
        self.asst_results_df['Opt-in Q Time'] = [0.0]
        ...
        # time allocated to completing TA
        self.asst_results_df['TA Mins'] = [0]
        ...
        # which Step2 pathway the patient was sent down
        self.step2_results_df['Route Name'] = ['NA']
        ...
        # which Step2 pathway the patient was sent down
        self.step2_sessions_df['Route Name'] = ['NA']
        self.step2_sessions_df['Session Number'] = [0]
        self.step2_sessions_df['Session Time'] = [
            0]  # clinical session time in mins
        self.step2_sessions_df['Admin Time'] = [
            0]  # admin session time in mins
        ...
        # which Step3 pathway the patient was sent down
        self.step3_results_df['Route Name'] = ['NA']
        ...
        # which Step2 pathway the patient was sent down
        self.step3_sessions_df['Route Name'] = ['NA']
        self.step3_sessions_df['Session Number'] = [0]
        self.step3_sessions_df['Session Time'] = [
            0]  # clinical session time in mins
        self.step3_sessions_df['Admin Time'] = [
            0]  # admin session time in mins
    ...
    def the_governor(self):
        ...
        while self.week_number < g.sim_duration:
            ...
            if g.debug_level >= 2:
                print(
                    f"Referral generator has returned {self.referrals_this_week} referrals")

            if g.debug_level >= 2:
                print(
                    f"Passing {self.referrals_this_week} to the patient treatment generator")
            ...
            if g.debug_level >= 2 and self.week_number < g.sim_duration:
                print(
                    f"Week {self.week_number-1} complete, moving on to Week {self.week_number}")
            elif g.debug_level >= 2 and self.week_number == g.sim_duration-1:
                print(
                    f"Week {self.week_number-1} complete, simulation has now finished")
    ...
    def patient_treatment_generator(self, number_of_referrals, treatment_week_number):
        ...
        if g.debug_level >= 2:
            print(
                f'Processing {number_of_referrals} referral through treatment')
        ...
        while self.referrals_to_process > 0:
            ...
            if g.debug_level >= 2:
                print(
                    f'Processing referral, {self.referrals_to_process} left out of {number_of_referrals}')
            ...
            if g.debug_level >= 2:
                print(
                    f'Referral processed, proceeding to next referral, {self.referrals_to_process} left')
    ...
    def weekly_stats(self, stats_week_number):
        ...
        for i, job_role in enumerate(self.job_role_list):
            ...
            self.staff_results_df_filtered = self.staff_results_df[
                ...
                (self.staff_results_df["Week Number"]
                 == self.stats_week_number)
            ]
        ...
        self.step2_weekly_waiting_stats['Weeks Waited'] = self.stats_week_number - \
            self.step2_weekly_waiting_stats['Start Week']
        ...
        for y, pathway in enumerate(self.waiting_list_path):
            ...
            self.step2_weekly_waiting_filtered = self.step2_weekly_waiting_stats[
                self.step2_weekly_waiting_stats['Route Name'] == pathway]
        ...
        self.step3_weekly_waiting_stats['Weeks Waited'] = self.stats_week_number - \
            self.step2_weekly_waiting_stats['Start Week']
        ...
        for z, pathway in enumerate(self.waiting_list_path):
            ...
            self.step3_weekly_waiting_filtered = self.step3_weekly_waiting_stats[
                self.step3_weekly_waiting_stats['Route Name'] == pathway]
    ...
    def caseload_builder(self):
        ...
        self.weekly_usage = {week: {"week_number": 0, "res_topup": 0}
                             for week in range(g.sim_duration)}

        # dictionary to keep track of resources to be restored
        self.pwp_restore = {f'{self.p_type}_{p}': {}
                            for p in range(g.number_staff_pwp)}
        ...
        # dictionary to keep track of resources to be restored
        self.cbt_restore = {f'{self.c_type}_{c}': {}
                            for c in range(g.number_staff_cbt)}
        ...
        # dictionary to keep track of resources to be restored
        self.couns_restore = {f'{self.s_type}_{s}': {}
                              for s in range(g.number_staff_couns)}
    ...
    def record_caseload_use(self, r_type, r_id, week_number):
        ...
        if self.restore_week not in self.resources_used[r_id]:
            self.resources_used[r_id][self.restore_week] = {
                "week_number": self.restore_week, "res_topup": 0}
        ...
        if g.debug_level >= 2:
            print(
                f"Resource {r_id} usage recorded for week {self.restore_week}: {self.resources_used[r_id][self.restore_week]}")
    ...
    def top_up_caseloads(self, week_number):
        ...
        for self.pwp_restore_id, week_data in self.pwp_restore.items():
            if self.week_num in week_data and week_data[self.week_num]['res_topup'] > 0:
                ...
                if g.debug_level >= 2:
                    print(
                        f"PwP Resource {self.pwp_restore_id} at week {self.week_num}: {self.pwp_resources[self.pwp_restore_id].level}, {self.pwp_topup_value} available to restore")

                self.pwp_resources[self.pwp_restore_id].put(
                    self.pwp_topup_value)

            if g.debug_level >= 2:
                print(
                    f"PwP Resource {self.pwp_restore_id} now at: {self.pwp_resources[self.pwp_restore_id].level}")
        ...
        for self.cbt_restore_id, week_data in self.cbt_restore.items():
            if self.week_num in week_data and week_data[self.week_num]['res_topup'] > 0:
                ...
                if g.debug_level >= 2:
                    print(
                        f"CBT Resource {self.cbt_restore_id} at week {self.week_num}: {self.cbt_resources[self.cbt_restore_id].level}, {self.cbt_topup_value} available to restore")

                self.cbt_resources[self.cbt_restore_id].put(
                    self.cbt_topup_value)

            if g.debug_level >= 2:
                print(
                    f"Couns Resource {self.cbt_restore_id} now at: {self.cbt_resources[self.cbt_restore_id].level}")
        ...
        for self.couns_restore_id, week_data in self.couns_restore.items():
            if self.week_num in week_data and week_data[self.week_num]['res_topup'] > 0:
                ...
                if g.debug_level >= 2:
                    print(
                        f"PwP Resource {self.couns_restore_id} at week {self.week_num}: {self.couns_resources[self.couns_restore_id].level}, {self.couns_topup_value} available to restore")

                self.couns_resources[self.couns_restore_id].put(
                    self.couns_topup_value)

            if g.debug_level >= 2:
                print(
                    f"PwP Resource {self.couns_restore_id} now at: {self.couns_resources[self.couns_restore_id].level}")
        # don't wait, go to the next step
        yield self.env.timeout(0)
    ...
    def find_caseload_slot(self, r_type):
        ...
        self.available_caseloads = {k: v for k,
                                    v in self.resources.items() if v.level > 0}
        ...
        while self.env.now < g.sim_duration:
            if self.available_caseloads:
                ...
                self.random_caseload_id = random.choice(
                    list(self.available_caseloads.keys()))
                self.selected_resource = self.available_caseloads[self.random_caseload_id]

                if g.debug_level >= 2:
                    print(
                        f'Resource {self.random_caseload_id} with a remaining caseload of {self.selected_resource.level} selected')
                # Return the ID and the container if available
                return self.random_caseload_id, self.selected_resource
                yield self.env.timeout(0)
            else:
                if g.debug_level >= 2:
    ...
    def pwp_staff_generator(self, week_number):
        ...
        while self.pwp_counter < g.number_staff_pwp:
            ...
            if self.week_number % 4 == 0:
                ...
                self.staff_results_df.at[s.id,
                                         'Supervision Mins'] = g.supervision_time/2
                self.staff_results_df.at[s.id,
                                         'Wellbeing Mins'] = g.wellbeing_time/2
    ...
    def couns_staff_generator(self, week_number):
        ...
        while self.couns_counter < g.number_staff_couns:
            ...
            # Couns only
            self.staff_results_df.at[s.id,
                                     'Huddle Mins'] = g.counsellors_huddle/2
    ...
    def screen_referral(self, asst_week_number):
        ...
        self.asst_results_df.at[p.id, 'Referral Time Screen'
            ...
            g.referral_screen_time, g.std_dev)
        ...
        if self.reject_referral > g.referral_rej_rate:
            ...
            self.asst_results_df.at[p.id,
                                    'Week Number'] = self.asst_week_number
            ...
            if g.debug_level >= 2:
                print(
                    f"Referral Accepted flag set to {self.asst_results_df.at[p.id,'Referral Accepted']} for Patient {p.id}")
    ...
    def review_referral(self, patient):
        ...
        if self.requires_review > g.referral_review_rate:
            ...
            if self.review_reject < g.review_rej_rate:
                ...
                self.asst_results_df.at[p.id,
                                        'Review Rejected'] = p.referral_review_rej

                if g.debug_level >= 2:
                    print(
                        f"Patient {p.id} Reviewed and Rejection flag set to {self.asst_results_df.at[p.id,'Review Rejected']}")

                # set flag to show Patient required review
                self.asst_results_df.at[p.id,
                                        'Referral Reviewed'] = p.referral_req_review
            ...
            if g.debug_level >= 2:
                print(
                    f"Patient {p.id} Did Not Require Review, Rejection flag set to {self.asst_results_df.at[p.id,'Review Rejected']}")

            if g.debug_level >= 2:
                print(
                    f"Patient {p.id} did not require a Review - go to opt-in")
    ...
    def telephone_assessment(self, patient):
        ...
        if g.debug_level >= 2:
            print(
                f"Patient {p.id} requesting TA resource, current res level {self.ta_res.level}")
        with self.ta_res.get(1) as ta_req:
            yield ta_req

        if g.debug_level >= 2:
            print(
                f"Patient {p.id} allocated TA resource, new res level {self.ta_res.level}")
        ...
        if g.debug_level >= 2:
            print(
                f'Week {self.env.now}: Patient number {p.id} (added week {p.week_added}) put through TA')
        ...
        if self.ta_accepted > g.ta_accept_rate:
            ...
            if self.ta_accepted == 1:
                ...
                self.step_options = random.choices(
                    g.step_routes, weights=g.step2_step3_ratio, k=self.referrals_this_week)
                ...
                self.asst_results_df.at[p.id,
                                        'Treatment Path'] = self.selected_step
    ...
    def pathway_runner(self, patient, step_chosen):
        ...
        if step_chosen == 'Step2':
            ...
            if g.debug_level >= 2:
                print(
                    f'PATHWAY RUNNER: Patient {p.id} sent down **{p.initial_step}** pathway')
            # yield self.env.timeout(0)
    ...
    def patient_step2_pathway(self, patient):
        ...
        if p.step2_path_route == 'PwP':
            ...
            self.step2_waiting_list.at[p.id,
        ...
        # self.step2_waiting_list['Start Week'] = 0
            ...
            if g.debug_level >= 2:
                ...
                if g.debug_level >= 2:
                    print(
                        f'Group store contains {len(self.group_store.items)} of possible {g.step2_group_size}')
                ...
                self.step2_waiting_list.at[p.id,
                                           'WL Position'] = g.number_on_pwp_wl
                self.step2_waiting_list.at[p.id,
                                           'Start Week'] = self.week_number

                if len(self.group_store.items) == 7:
                    if g.debug_level >= 2:
                        print(
                            f'Group is now full, putting {len(self.group_store.items)} through group therapy')
                    ...
                    while len(self.group_store.items) > 0:
                        ...
                        if g.debug_level >= 2:
                            print(
                                f'Putting Patient {p.id} through Group Therapy, {len(self.group_store.items)} remaining')
                        if g.debug_level >= 2:
                            print(
                                f"FUNC PROCESS patient_step2_pathway: Patient {p.id} Initiating {p.step2_path_route} Step 2 Route")
                        # yield self.env.timeout(0)
    ...
    def patient_step3_pathway(self, patient):
        ...
        if p.step3_path_route == 'CBT':
            ...
            self.step3_waiting_list.at[p.id,
                                       'WL Position'] = g.number_on_cbt_wl
            self.step3_waiting_list.at[p.id, 'Start Week'] = self.week_number

            if g.debug_level >= 2:
                print(
                    f"FUNC PROCESS patient_step3_pathway: Patient {p.id} Initiating {p.step3_path_route} Step 3 Route")
    ...
    def step2_pwp_process(self, patient):
        ...
        if g.debug_level >= 2:
            print(
                f'{p.step2_path_route} RUNNER: Patient {p.id} added to {p.step2_path_route} waiting list')
        ...
        while True:
            ...
            if self.result == (None, None):
                print(
                    f"Stopping retry as no resources are available. Time: {self.env.now}")
        ...
        if g.debug_level >= 2:
            print(
                f'Resource {self.pwp_caseload_id} with a caseload remaining of {self.pwp_caseload_res.level} allocated to patient {p.id}')
        ...
        if g.debug_level >= 2:
            print(
                f'Patient {p.id} added to caseload {p.step2_resource_id} spaces left')
        ...
        if g.debug_level >= 2:
            print(
                f'{p.step2_path_route} RUNNER: Patient {p.id} removed from {p.step2_path_route} waiting list')

        if g.debug_level >= 2:
            print(
                f'FUNC PROCESS step2_pwp_process: Week {self.env.now}: Patient {p.id} (added week {p.week_added}) put through {p.step2_path_route}')
        ...
        self.pwp_random_weeks = random.sample(range(
            self.week_number+1, self.week_number+g.step2_pwp_period), g.step2_pwp_sessions)
        ...
        if g.debug_level >= 2:
            print(
                f'Random Session week {len(self.pwp_random_weeks)} numbers are {self.pwp_random_weeks}')
        ...
        while self.pwp_session_counter < g.step2_pwp_sessions and self.pwp_dna_counter < self.dnas_allowed:
            ...
            is_step_up = 1 if self.pwp_session_counter >= g.step2_pwp_sessions - \
                1 and self.step_patient_up <= g.step_up_rate else 0
            if is_step_up == 1:
            ...
            self.step2_sessions_df = pd.concat(
                [self.step2_sessions_df, pd.DataFrame([new_row])], ignore_index=True)
            ...
            if is_step_up:
                ...
                if g.debug_level >= 2:
                    print(
                        f'### STEPPED UP ###: Patient {p.id} has been stepped up, running Step3 route selector')
                yield self.env.process(self.patient_step3_pathway(p))
            ...
            if is_dropout:
                ...
                if g.debug_level >= 2:
                    print(
                        f'Patient {p.id} dropped out of {p.step2_path_route} treatment')
                p.step2_end_week = p.step2_start_week + \
                    self.pwp_random_weeks[self.pwp_session_counter]
        ...
        if self.pwp_dna_counter >= self.dnas_allowed:
            ...
            self.env.process(self.record_caseload_use(
                p.step2_path_route, self.pwp_caseload_id, self.pwp_random_weeks[self.pwp_session_counter]))

        else:
            # record when the caseload resource can be restored
            self.env.process(self.record_caseload_use(
                p.step2_path_route, self.pwp_caseload_id, max(self.pwp_random_weeks)))
    ...
    def step2_group_process(self, patient):
        ...
        if g.debug_level >= 2:
            print(
                f'FUNC PROCESS step2_group_process: Week {self.env.now}: Patient {p.id} (added week {p.week_added}) put through {p.step2_path_route}')
        ...
        # Generate a list of week numbers the patient is going to attend
        self.group_random_weeks = random.sample(
            range(self.week_number+1, self.week_number+10), g.step2_group_sessions)
        ...
        while self.group_session_counter < g.step2_group_sessions and self.group_dna_counter < self.dnas_allowed:
            ...
            if is_dna:
                ...
                session_time = int(
                    g.step2_group_session_mins/g.step2_group_size)
                admin_time = g.step2_session_admin

            # Determine if the patient is stepped up
            self.step_patient_up = random.uniform(0, 1)
            is_step_up = 1 if self.group_session_counter >= g.step2_pwp_sessions - \
                1 and self.step_patient_up <= g.step_up_rate else 0
            if is_step_up == 1:
                self.step2_results_df.at[p.id, 'IsStep'] = 1
            else:
            ...
            if is_dropout:
                ...
                if g.debug_level >= 2:
                    print(
                        f'Patient {p.id} dropped out of {p.step2_path_route} treatment')
                p.step2_end_week = p.step2_start_week + \
                    self.group_random_weeks[self.group_session_counter]
                break  # Stop the loop if patient drops out
    ...
    def step3_cbt_process(self, patient):
        ...
        if g.debug_level >= 2:
            print(
                f'{p.step3_path_route} RUNNER: Patient {p.id} added to {p.step3_path_route} waiting list')
        ...
        if g.debug_level >= 2:
            print(
                f'Resource {self.cbt_caseload_id} with a caseload remaining of {self.cbt_caseload_res.level} allocated to patient {p.id}')

        if g.debug_level >= 2:
            print(
                f'Patient {p.id} added to caseload {p.step3_resource_id}, {self.cbt_resources[p.step3_resource_id].level} spaces left')
        ...
        if g.debug_level >= 2:
            print(
                f'{p.step3_path_route} RUNNER: Patient {p.id} removed from {p.step3_path_route} waiting list')

        if g.debug_level >= 2:
            print(
                f'FUNC PROCESS step3_cbt_process: Week {self.env.now}: Patient {p.id} (added week {p.week_added}) put through {p.step3_path_route}')
        ...
        if self.vary_step3_sessions >= g.step_3_session_var:
            # use the standard number of sessions
            self.number_cbt_sessions = g.step3_cbt_sessions
            self.step3_cbt_period = g.step3_cbt_period  # use the standard delivery period
        else:
            # use the randomly generated number of sessions
            self.number_cbt_sessions = self.random_num_sessions
            self.step3_cbt_period = g.step3_cbt_period + \
                (self.random_num_sessions*2)

        # Generate a list of week numbers the patient is going to attend
        self.cbt_random_weeks = random.sample(range(
            self.week_number+1, self.week_number+(self.step3_cbt_period*2)), self.number_cbt_sessions)
        # Add 1 at the start of the list
        ...
        if g.debug_level >= 2:
            print(
                f'Random Session week {len(self.cbt_random_weeks)} numbers are {self.cbt_random_weeks}')
        ...
        while self.cbt_session_counter < g.step3_cbt_sessions and self.cbt_dna_counter < self.dnas_allowed:
            ...
            is_step_down = 1 if self.cbt_session_counter >= g.step3_cbt_sessions - \
                1 and self.step_patient_down <= g.step_down_rate else 0
            if is_step_down == 1:
            ...
            self.step3_sessions_df = pd.concat(
                [self.step3_sessions_df, pd.DataFrame([new_row])], ignore_index=True)
            ...
            if is_dropout:
                ...
                if g.debug_level >= 2:
                    print(
                        f'Patient {p.id} dropped out of {p.step3_path_route} treatment')
                p.step3_end_week = p.step3_start_week + \
                    self.cbt_random_weeks[self.cbt_session_counter]
                break  # Stop the loop if patient drops out
        ...
        if self.cbt_dna_counter >= self.dnas_allowed:
            ...
            self.env.process(self.record_caseload_use(
                p.step3_path_route, self.cbt_caseload_id, self.cbt_random_weeks[self.cbt_session_counter]))

        else:
            # record when the caseload resource can be restored
            self.env.process(self.record_caseload_use(
                p.step3_path_route, self.cbt_caseload_id, max(self.cbt_random_weeks)))
    ...
    def step3_couns_process(self, patient):
        ...
        if g.debug_level >= 2:
            print(
                f'Resource {self.couns_caseload_id} with a caseload remaining of {self.couns_caseload_res.level} allocated to patient {p.id}')
        ...
        if g.debug_level >= 2:
            print(
                f'Patient {p.id} added to caseload {p.step3_resource_id}, {self.couns_resources[p.step3_resource_id].level} spaces left')
        ...
        if g.debug_level >= 2:
            print(
                f'FUNC PROCESS step3_couns_process: Week {self.env.now}: Patient {p.id} (added week {p.week_added}) put through {p.step3_path_route}')
        ...
        # Generate a list of week numbers the patient is going to attend
        self.couns_random_weeks = random.sample(range(
            self.week_number+1, self.week_number+(self.step3_couns_period*2)), self.number_couns_sessions)
        ...
        if self.couns_session_counter < len(self.couns_random_weeks):
            p.step3_end_week = p.step3_start_week + \
                self.couns_random_weeks[self.couns_session_counter]
        else:
            if g.debug_level >= 2:
                print(
                    f"Warning: Index {self.couns_session_counter} out of range for couns_random_weeks.")

        if g.debug_level >= 2:
            print(
                f'Random Session week {len(self.couns_random_weeks)} numbers are {self.couns_random_weeks}')
        ...
        while self.couns_session_counter < g.step3_couns_sessions and self.couns_dna_counter < self.dnas_allowed:
            ...
            # Determine if the patient is stepped up
            self.step_patient_down = random.uniform(0, 1)
            is_step_down = 1 if self.couns_session_counter >= g.step3_couns_sessions - \
                1 and self.step_patient_down <= g.step_down_rate else 0
            if is_step_down == 1:
                self.step3_results_df.at[p.id, 'IsStep'] = 1
            else:
        ...
        if self.couns_dna_counter >= self.dnas_allowed:
            ...
            self.env.process(self.record_caseload_use(
                p.step3_path_route, self.couns_caseload_id, self.couns_random_weeks[self.couns_session_counter]))

        else:
            # record when the caseload resource can be restored
            self.env.process(self.record_caseload_use(
                p.step3_path_route, self.couns_caseload_id, max(self.couns_random_weeks)))
