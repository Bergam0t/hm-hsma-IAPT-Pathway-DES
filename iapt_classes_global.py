...
class g:
    ...
    # [0.94,0.06] # Step2 proportion for each route
    step2_path_ratios = [0.8, 0.2]
    ...
    # full pathway options = ['PfCBT','Group','CBT','EMDR','DepC','DIT','IPT','CDEP']
    step3_routes = ['Couns', 'CBT']
    # [0.1,0.25,0.25,0.05,0.05,0.1,0.1,0.1] # Step3 proportion for each route ##### Need to clarify exact split
    step3_path_ratios = [0.368, 0.632]
    ...
    # % of instances where number sessions goes over standard amount
    step_3_session_var = 0.15

    # Staff
    # 2 hours per month per modality ##### could use modulus for every 4th week
    supervision_time = 120
    ...
    # job plan = 1 group per week per PwP, assume 12 per group
    group_resource = number_staff_pwp
    ...
    # job plan = 2 x 1st + 20 X FUP per cbt per week
    couns_resource = number_staff_couns
    couns_caseload = 25
    ...
    referral_rate_lookup = pd.read_csv(
        'talking_therapies_referral_rates.csv', index_col=0)
